...
func SilenceEndsAt(silence v1alpha1.Silence) (time.Time, error) {
	...
	if err != nil {
		...
		expirationDate, err = time.Parse(time.DateOnly, value)
		if err != nil {
			return time.Time{}, microerror.Maskf(invalidValidUntilDateError, "%s date %q does not match expected format %q: %v", ValidUntilAnnotationName, value, time.DateOnly, err)
		}
